...
The engine classifies each run into one of these failure modes:
 
`CASCADING_FAILURE` · `RETRY_STORM` · `CONNECTION_POOL_EXHAUSTION` · `TIMEOUT_MISCONFIGURATION` · `HEALTHY` · `UNKNOWN`
...
```json
{
  ...
  "evidence": [
    "orders-service connection pool at 100%",
    "api-gateway p99 jumped to 1450ms"
  ],
  "suggested_fix": {
    "action": "Add Circuit Breaker & Bulkhead",
    "target_service": "orders-service",
    "recommendation": "Enforce a 400ms timeout on downstream calls and isolate connection pool capacity."
  }
}
```
---
...
![Live Topology and Reasoning](resource/img1.png)
![Healthy Diagnosis](resource/healthy.png)
![Chaos Injection Active](resource/buggy.png)
